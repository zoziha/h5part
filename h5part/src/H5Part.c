...
)
{
    ...
    if (f->xfer_prop != H5P_DEFAULT)
    {
        ...
        if (r < 0)
            HANDLE_H5P_CLOSE_ERR("f->xfer_prop");
    ...
    if (f->dcreate_prop != H5P_DEFAULT)
    {
        ...
        if (r < 0)
            HANDLE_H5P_CLOSE_ERR("f->dcreate_prop");
    ...
    if (f->access_prop != H5P_DEFAULT)
    {
        ...
        if (r < 0)
            HANDLE_H5P_CLOSE_ERR("f->access_prop");
    ...
    if (f->fcreate_prop != H5P_DEFAULT)
    {
        ...
        if (r < 0)
            HANDLE_H5P_CLOSE_ERR("f->fcreate_prop");
    ...
    const h5part_int64_t _stride)
{
    ...
    */
    ...
    if (f->shape < 0)
        HANDLE_H5S_CREATE_SIMPLE_ERR(count);
    f->viewstart = 0;
    ...
    */
    ...
    f->shape = H5Screate_simple(1, &count, NULL);
    if (f->shape < 0)
        return HANDLE_H5S_CREATE_SIMPLE_ERR(count);
...
)
{
    ...
    if (herr < 0)
        return HANDLE_H5D_WRITE_ERR(name2, f->timestep);
    ...
    void *attrib_value)
{
...
#endif
    if (attrib_id <= 0)
        return HANDLE_H5A_OPEN_NAME_ERR(attrib_name);

    type_id = H5Aget_type(attrib_id);
    if (type_id < 0)
        return HANDLE_H5A_GET_TYPE_ERR;

    space_id = H5Aget_space(attrib_id);
    if (space_id < 0)
        return HANDLE_H5A_GET_SPACE_ERR;

    herr = H5Aread(attrib_id, type_id, attrib_value);
    if (herr < 0)
        return HANDLE_H5A_READ_ERR;
    ...
    if (herr < 0)
        return HANDLE_H5T_CLOSE_ERR;

    herr = H5Aclose(attrib_id);
    if (herr < 0)
        return HANDLE_H5A_CLOSE_ERR;
    ...
    const hsize_t attrib_nelem)
{
    ...
    if (attrib_id < 0)
        return HANDLE_H5A_CREATE_ERR(attrib_name);

    herr = H5Awrite(attrib_id, type, attrib_value);
    if (herr < 0)
        return HANDLE_H5A_WRITE_ERR(attrib_name);
    ...
    h5part_int64_t *attrib_nelem)
{
    ...
    if (attrib_id < 0)
        return HANDLE_H5A_OPEN_IDX_ERR(attrib_idx);
    ...
    if (attrib_name)
    {
        herr = H5Aget_name(
            ...
            attrib_name);
        if (herr < 0)
            return HANDLE_H5A_GET_NAME_ERR;
    }
    if (attrib_type)
    {
...
)
{
    ...
    case H5L_TYPE_EXTERNAL:
    {
        ...
        if (targbuf == NULL)
            return (herr_t)HANDLE_H5PART_NOMEM_ERR;
...
)
{
    ...
    if (_H5Part_has_view(f))
    {
        ...
        if (nparticles < 0)
            return HANDLE_H5S_GET_SELECT_NPOINTS_ERR;
...
)
{
    ...
    */
    ...
    if (start == -1)
        start = 0;
    if (end == -1)
        end = total - 1; // range is *inclusive*
...
)
{
    ...
    */
    ...
    if (herr < 0)
        return HANDLE_H5S_SELECT_ELEMENTS_ERR;
...
)
{
    ...
    herr = _reset_view(f);
    if (herr < 0)
        return HANDLE_H5PART_SET_VIEW_ERR(herr, -1, -1);
	...
	*/
    ...
    if (herr < 0)
        return HANDLE_H5PART_SET_VIEW_ERR(herr, start, end);
    ...
    const hid_t type)
{
    ...
    if (dataset_id < 0)
        return HANDLE_H5D_OPEN_ERR(name2);
    ...
    herr = H5Dread(
        ...
        array);
    if (herr < 0)
        return HANDLE_H5D_READ_ERR(name2, f->timestep);
    ...
    if (f->throttle > 0)
    {
        ...
        if (f->myproc / f->throttle > 0)
        {
            ...
            ret = MPI_Recv(
                ...
                MPI_STATUS_IGNORE);
            if (ret != MPI_SUCCESS)
                return HANDLE_MPI_SENDRECV_ERR;
        }
        _H5Part_print_debug_detail(
...
void _H5Part_print_error(
    const char *fmt,
    ...)
{

    if (_debug < 1 || !_is_root_proc)
        return;
    va_list ap;
    va_start(ap, fmt);
    _vprint(stderr, "E", fmt, ap);
...
void _H5Part_print_warn(
    const char *fmt,
    ...)
{

    if (_debug < 2 || !_is_root_proc)
        return;
    va_list ap;
    va_start(ap, fmt);
...
void _H5Part_print_info(
    const char *fmt,
    ...)
{

    if (_debug < 3 || !_is_root_proc)
        return;
    va_list ap;
    va_start(ap, fmt);
...
void _H5Part_print_debug(
    const char *fmt,
    ...)
{

    if (_debug < 4 || !_is_root_proc)
        return;
    va_list ap;
    va_start(ap, fmt);
...
void _H5Part_print_debug_detail(
    const char *fmt,
    ...)
{
