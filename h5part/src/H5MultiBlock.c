...
)
{
    ...
    if (i == 0)
        mb->field_edges |= H5MB_EDGE_Z0;
    if (i == mb->decomp[0] - 1)
        mb->field_edges |= H5MB_EDGE_Z1;
    if (j == 0)
        mb->field_edges |= H5MB_EDGE_Y0;
    if (j == mb->decomp[1] - 1)
        mb->field_edges |= H5MB_EDGE_Y1;
    if (k == 0)
        mb->field_edges |= H5MB_EDGE_X0;
    if (k == mb->decomp[2] - 1)
        mb->field_edges |= H5MB_EDGE_X1;
    ...
    const char *edges)
{
    ...
    if ((h5part_int64_t)ibufsize != bufsize)
    ...
    const char *edges)
{
    ...
    icount = (int)count;
    if ((h5part_int64_t)icount != count)
        return HANDLE_MPI_INT64_ERR;
...
)
{
    ...
    {
        count = 1;
        blocklen = mb->halo_dims[2] * mb->halo_dims[1] * mb->halo_radii[0] * typesize;
        stride = mb->halo_dims[2] * mb->halo_dims[1] * mb->halo_dims[0] * typesize;
...
)
{
    ...
    herr = H5Dread(
        ...
        *data);
    if (herr < 0)
        return HANDLE_H5D_READ_ERR(field_name, f->timestep);
