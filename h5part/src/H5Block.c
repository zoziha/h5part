...
)
{
    ...
    if (herr < 0)
        return HANDLE_H5P_CLOSE_ERR("create_prop");
...
)
{
    ...
    {
        if (partition->i_end > b->i_max)
            b->i_max = partition->i_end;
        if (partition->j_end > b->j_max)
            b->j_max = partition->j_end;
        if (partition->k_end > b->k_max)
            b->k_max = partition->k_end;
    }
}

#define _NO_GHOSTZONE(p, q) ((p->i_end < q->i_start) || (p->j_end < q->j_start) || (p->k_end < q->k_start))
...
{
    return (p->i_end - p->i_start) * (p->j_end - p->j_start) * (p->k_end - p->k_start);
}
...
)
{
    ...
    h5part_int64_t dx = MIN(p->i_end, q->i_end) - MAX(p->i_start, q->i_start) + 1;
    h5part_int64_t dy = MIN(p->j_end, q->j_end) - MAX(p->j_start, q->j_start) + 1;
    h5part_int64_t dz = MIN(p->k_end, q->k_end) - MAX(p->k_start, q->k_start) + 1;
...
)
{
    ...
    if (herr < 0)
        return HANDLE_H5PART_LAYOUT_ERR;
...
)
{
    ...
    if (herr < 0)
        return HANDLE_H5P_SET_CHUNK_ERR;
...
)
{
    ...
    if (dataset_id < 0)
        return HANDLE_H5D_OPEN_ERR(field_name);

    hid_t plist_id = H5Dget_create_plist(dataset_id);
    if (plist_id < 0)
        return HANDLE_H5D_GET_PLIST_ERR;

    hsize_t cdims[3];
    int ndim = H5Pget_chunk(plist_id, 3, cdims);
    if (ndim != 3)
        return HANDLE_H5P_GET_CHUNK_ERR;
...
)
{
    ...
    if (b->blockgroup < 0)
    {
        ...
        if (herr < 0)
            return HANDLE_H5G_OPEN_ERR(H5BLOCK_GROUPNAME_BLOCK);
        b->blockgroup = herr;
    }
    ...
    const char *name)
{
    ...
    b->field_group_id = H5Gopen(b->blockgroup, name
#ifndef H5_USE_16_API
                                ,
                                H5P_DEFAULT
#endif
    );
    if (b->field_group_id < 0)
    ...
    hid_t dataset)
{
    ...
    if (rank < 0)
        return HANDLE_H5S_GET_SIMPLE_EXTENT_DIMS_ERR;
    if (rank != 3)
        return HANDLE_H5PART_DATASET_RANK_ERR(rank, 3);
    ...
    herr = H5Sselect_hyperslab(
        ...
        NULL);
    if (herr < 0)
        return HANDLE_H5S_SELECT_HYPERSLAB_ERR;
...
)
{
    ...
    if (dataset_id < 0)
        return HANDLE_H5D_OPEN_ERR(name);
    ...
    herr = H5Dread(
        ...
        data);
    if (herr < 0)
        return HANDLE_H5D_READ_ERR(name, f->timestep);
...
)
{
    ...
    if (f->block->blockgroup < 0)
        return HANDLE_H5G_CREATE_ERR(H5BLOCK_GROUPNAME_BLOCK);
...
)
{
    ...
    if (exists > 0)
        return HANDLE_H5D_EXISTS_ERR(name, f->timestep);
#endif
    ...
    herr = H5Dwrite(
        ...
        data);
    if (herr < 0)
        return HANDLE_H5D_WRITE_ERR(name, f->timestep);
...
)
{
    ...
    if (group_id < 0)
        return HANDLE_H5G_OPEN_ERR(field_name);
    ...
    if (_grid_rank < 0)
        return HANDLE_H5S_GET_SIMPLE_EXTENT_DIMS_ERR;
    if (grid_rank)
        *grid_rank = (h5part_int64_t)_grid_rank;
    ...
    if (_field_rank < 0)
        return *field_rank;
    if (field_rank)
        *field_rank = (h5part_int64_t)_field_rank;

    hid_t h5type = H5Dget_type(dataset_id);
    if (h5type < 0)
        return HANDLE_H5D_GET_TYPE_ERR;
